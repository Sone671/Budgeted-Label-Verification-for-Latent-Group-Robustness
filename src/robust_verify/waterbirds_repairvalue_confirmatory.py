"""Frozen integrity rules for the Waterbirds RepairValue-Q E2E confirmation.

This external-validity protocol is prospectively frozen after the CelebA
RepairValue-Q confirmation and the separate Waterbirds seed 10--19 mechanism
follow-up.  It uses fresh Waterbirds corruption seeds 20--39.  Those earlier
blocks must never be pooled with this one.  Only filesystem paths may be
...
PROTOCOL_VERSION = "waterbirds-e2e-repairvalue-confirmatory-seeds-20-39-v1"
FROZEN_SEEDS = tuple(range(20, 40))
FROZEN_METHODS = (
    "random",
    "loss",
    "noise_score",
...
def validate_frozen_config(
    ...
    expected: dict[str, Any] = {
        ...
        "confirmation.role": "prospective_external_validity_confirmation",
        "confirmation.hypothesis_origin": (
            "celeba_repairvalue_seed20_29_and_waterbirds_mechanism_seed10_19"
        ),
    ...
    wanted_options = {"repair_value_tail_fraction": REPAIR_VALUE_TAIL_FRACTION}
...
def summarize_confirmatory_results(frame: pd.DataFrame) -> dict[str, Any]:
    """Apply the registered Waterbirds RepairValue success gates."""
    ...
    noise_score_paired = _paired_effect(frame, PRIMARY_METHOD, "noise_score")
    tracin_paired = _paired_effect(frame, "tracin_val_uncertainty", PAIRED_BASELINE)
    ...
    noise_score_ci = bootstrap_mean_ci(noise_score_paired, rng, BOOTSTRAP_REPLICATES)
    tracin_ci = bootstrap_mean_ci(tracin_paired, rng, BOOTSTRAP_REPLICATES)
    ...
    return {
        ...
        "tracin_vs_loss_secondary": {
            "mean_delta_wga_difference": float(tracin_paired.mean()),
            "bootstrap_95_ci": [float(tracin_ci[0]), float(tracin_ci[1])],
        },
        "repairvalue_vs_random_secondary": {
        ...
        "repairvalue_vs_noise_score_secondary": {
            "mean_delta_wga_difference": float(noise_score_paired.mean()),
            "bootstrap_95_ci": [float(noise_score_ci[0]), float(noise_score_ci[1])],
        },
        "primary_gate": gates,
