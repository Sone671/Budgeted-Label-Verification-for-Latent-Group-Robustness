"""Integrity rules for the CelebA Young/Male end-to-end replication.

This protocol is deliberately separate from the frozen Eyeglasses/Male
protocols.  It fixes a new target/spurious task, fresh corruption seeds, and
the five practical query methods used by the replication.  The module keeps
the public/private manifest boundary auditable and validates the additional
OOF Cleanlab artifact produced by the end-to-end runner.
"""
...
FROZEN_SEEDS = tuple(range(60, 80))
FROZEN_METHODS = (
    "random",
    "loss",
    "noise_score",
...
OOF_ARTIFACT_COLUMN = "oof_probability_artifact"
...
def validate_frozen_config(
    ...
    """Reject scientific changes to the Young/Male replication protocol."""
    expected: dict[str, Any] = {
        "confirmation.protocol_version": PROTOCOL_VERSION,
        "confirmation.role": "prospective_task_level_replication",
        "confirmation.hypothesis_origin": "preregistered_before_formal_run",
        "confirmation.primary_method": PRIMARY_METHOD,
        "confirmation.paired_baseline": PAIRED_BASELINE,
        "confirmation.primary_estimand": "mean_seed_paired_delta_wga_difference_vs_loss",
        ...
        "data.dataset": "celeba",
        "data.target_attr": "Young",
        "data.spurious_attr": "Male",
        "data.image_subdir": "img_align_celeba",
        "data.minority_mode": "label_neq_place",
        ...
        **{f"end_to_end.{name}": value for name, value in FROZEN_E2E.items()},
        "analysis.reference_method": "loss",
        "analysis.methods": ["noise_score", "oof_cleanlab", "expected_repair_value"],
        "analysis.bootstrap_replicates": BOOTSTRAP_REPLICATES,
        "analysis.rng_seed": BOOTSTRAP_RNG_SEED,
    }
    problems: list[str] = []
    ...
    if _get(config, "noise.settings") != wanted_noises:
        problems.append(f"noise.settings: expected {wanted_noises!r}, found {_get(config, 'noise.settings')!r}")
    ...
    wanted_options = {"repair_value_tail_fraction": REPAIR_VALUE_TAIL_FRACTION, **OOF_OPTIONS}
    if options != wanted_options:
        problems.append(f"experiment.baseline_options: expected {wanted_options!r}, found {options!r}")

    if problems:
        raise ValueError("Frozen CelebA Young/Male protocol mismatch:\n- " + "\n- ".join(problems))


def protocol_payload(config: dict[str, Any]) -> dict[str, Any]:
    """Return the path-independent scientific payload used for hashing."""
...
def verify_preparation_manifest(
    ...
    for record in manifest.get("files", []):
        ...
        try:
            ...
            raise ValueError(f"Prepared artifact escapes source root: {path}") from exc
        if not path.is_file():
...
def validate_result_frame(frame: pd.DataFrame, *, expected_seeds: Iterable[int]) -> None:
    """Validate the complete method/seed key space and metric invariants."""
    ...
    if frame.duplicated(["seed", "method", "noise_name", "budget_fraction"]).any():
        raise ValueError("Duplicate Young/Male replication result keys")
    ...
    if not np.isfinite(frame[numeric].to_numpy(dtype=float)).all():
        raise ValueError("Replication results contain non-finite values")
    if not np.allclose(
...
def _validate_oof_artifact(
    path: Path, *, sample_ids: np.ndarray, seed: int, folds: int
) -> None:
    with np.load(path, allow_pickle=False) as payload:
        required = {"sample_ids", "probabilities", "fold_ids", "seed", "folds"}
        if set(payload.files) != required:
            raise ValueError(f"Unexpected OOF artifact keys: {path}")
        stored_ids = payload["sample_ids"].astype(np.int64, copy=False)
        probabilities = payload["probabilities"].astype(np.float32, copy=False)
        fold_ids = payload["fold_ids"].astype(np.int64, copy=False)
        if int(np.asarray(payload["seed"]).item()) != int(seed):
            raise ValueError(f"OOF artifact seed mismatch: {path}")
        if int(np.asarray(payload["folds"]).item()) != int(folds):
            raise ValueError(f"OOF artifact fold count mismatch: {path}")
    expected_ids = np.asarray(sample_ids, dtype=np.int64)
    if not np.array_equal(stored_ids, expected_ids):
        raise ValueError(f"OOF artifact sample IDs do not match source: {path}")
    if probabilities.shape != (len(expected_ids), 2):
        raise ValueError(f"OOF artifact probability shape mismatch: {path}")
    if fold_ids.shape != (len(expected_ids),):
        raise ValueError(f"OOF artifact fold shape mismatch: {path}")
    if not np.all(np.isfinite(probabilities)):
        raise ValueError(f"OOF artifact contains non-finite probabilities: {path}")
    if not np.allclose(probabilities.sum(axis=1), 1.0, atol=1e-5):
        raise ValueError(f"OOF artifact probabilities do not sum to one: {path}")
    if set(np.unique(fold_ids)) != set(range(folds)):
        raise ValueError(f"OOF artifact has incomplete fold IDs: {path}")
    if np.any(np.bincount(fold_ids, minlength=folds) == 0):
        raise ValueError(f"OOF artifact has an empty fold: {path}")
...
def validate_seed_artifacts(
    ...
    for row in frame.to_dict("records"):
        ...
        with np.load(prediction_path, allow_pickle=False) as payload:
            if set(payload.files) != {"sample_ids", "predictions"}:
                raise ValueError(f"Unexpected prediction artifact keys: {prediction_path}")
        ...
        with np.load(baseline_prediction_path, allow_pickle=False) as payload:
            if set(payload.files) != {"sample_ids", "predictions"}:
                raise ValueError(f"Unexpected baseline artifact keys: {baseline_prediction_path}")
        ...
        for name, expected in baseline_metrics.items():
            if f"baseline_{name}" in row:
                _assert_close(f"seed {seed}/{method}/baseline_{name}", row[f"baseline_{name}"], expected)
        ...
        verified_files.update(
            {query_path, prediction_path, baseline_prediction_path, checkpoint_path, baseline_checkpoint_path}
        )

        oof_value = str(row.get(OOF_ARTIFACT_COLUMN, ""))
        if method == "oof_cleanlab" and not oof_value:
            raise ValueError(f"Seed {seed}/oof_cleanlab is missing its OOF artifact")
        if oof_value:
            oof_path = _artifact_path(shard, oof_value)
            _validate_oof_artifact(
                oof_path,
                sample_ids=public["sample_id"].to_numpy(dtype=np.int64),
                seed=seed,
                folds=int(OOF_OPTIONS["oof_folds"]),
            )
            oof_paths.add(oof_path)
            verified_files.add(oof_path)
    ...
    if len(oof_paths) != 1:
        raise ValueError(f"Seed {seed} must expose exactly one shared OOF artifact")
    return {
        "seed": seed,
        "rows": len(frame),
        "budget_count": expected_budget_count,
        "oof_artifact": str(next(iter(oof_paths))),
...
def _paired_effect(frame: pd.DataFrame, method: str, baseline: str) -> np.ndarray:
    ...
    if len(paired) != len(left) or len(paired) != len(right):
        raise ValueError(f"Cannot form a complete paired contrast: {method} vs {baseline}")
...
def summarize_confirmatory_results(frame: pd.DataFrame) -> dict[str, Any]:
    """Apply the registered RV-Q-vs-Loss and absolute utility gates."""
    ...
    for baseline in ("loss", "noise_score", "oof_cleanlab"):
        values = _paired_effect(frame, PRIMARY_METHOD, baseline)
        ci = bootstrap_mean_ci(values, rng, BOOTSTRAP_REPLICATES)
        paired_summaries[baseline] = {
            "mean_delta_wga_difference": float(values.mean()),
            ...
            "exact_two_sided_sign_test_p": exact_two_sided_sign_test(values),
        }
    loss_summary = paired_summaries[PAIRED_BASELINE]
    gates = {
        "paired_bootstrap_ci_lower_above_zero": bool(
            loss_summary["bootstrap_95_ci"][0] > 0
        ),
        "positive_paired_seed_count_at_least_15": bool(
            loss_summary["positive_seed_count"] >= PRIMARY_MIN_POSITIVE_PAIRED_SEEDS
        ),
    ...
    return {
        ...
        "seeds": list(FROZEN_SEEDS),
        "primary_estimand": "mean paired delta_wga(RepairValue-Q) minus delta_wga(Loss)",
        "paired_contrasts": paired_summaries,
        "paired_vs_loss": loss_summary,
        "absolute_utility": {
            ...
            "mean_noise_precision": float(candidate["noise_precision"].mean()),
...
def build_group_mechanism_audit(
    ...
    """Audit each method's coverage of its same-seed baseline worst group."""
    validate_result_frame(frame, expected_seeds=FROZEN_SEEDS)
    ...
    for row in frame.sort_values(["seed", "method"]).to_dict("records"):
        ...
        baseline_weak_group = min(
            group_ids,
            key=lambda group: (float(row[f"baseline_group_{group}_accuracy"]), group),
        )
        post_worst_group = min(
            group_ids,
            key=lambda group: (float(row[f"group_{group}_accuracy"]), group),
        )
        private = pd.read_csv(
