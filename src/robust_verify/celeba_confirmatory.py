"""Frozen protocol and integrity checks for the held-out CelebA E2E run.

The scientific knobs in this module are intentionally constants.  Server
paths may change, but seeds, data semantics, corruption, training settings,
query methods, budget, and the decision rule may not be changed under the same
protocol version.
"""
...
FROZEN_SEEDS = tuple(range(10, 20))
FROZEN_METHODS = ("loss", "tracin_val_uncertainty")
FROZEN_BUDGET = 0.10
FROZEN_NOISE_NAME = "uniform20"
PRIMARY_METHOD = "tracin_val_uncertainty"
PRIMARY_MIN_NEGATIVE_SEEDS = 8
PRIMARY_MIN_MEAN_NOISE_PRECISION = 0.99
...
def validate_frozen_config(
    ...
    """Reject any scientific change to the registered confirmatory protocol."""
    expected: dict[str, Any] = {
        "confirmation.protocol_version": PROTOCOL_VERSION,
        "confirmation.role": "independent_confirmatory",
        "confirmation.primary_method": PRIMARY_METHOD,
        "confirmation.primary_gate.min_negative_seeds": PRIMARY_MIN_NEGATIVE_SEEDS,
        "confirmation.primary_gate.min_mean_noise_precision": (
            PRIMARY_MIN_MEAN_NOISE_PRECISION
        ),
        "confirmation.primary_gate.require_bootstrap_ci_upper_below_zero": True,
        "data.dataset": "celeba",
        "data.target_attr": "Eyeglasses",
        "data.spurious_attr": "Male",
        "data.image_subdir": "img_align_celeba",
        "data.minority_mode": "group_id",
        "data.minority_group": 3,
    ...
    if budgets != (FROZEN_BUDGET,):
        problems.append(
            f"experiment.budgets: expected {(FROZEN_BUDGET,)!r}, found {budgets!r}"
        )
    ...
    if problems:
        raise ValueError("Frozen CelebA protocol mismatch:\n- " + "\n- ".join(problems))


def protocol_payload(config: dict[str, Any]) -> dict[str, Any]:
    """Return the path-independent scientific protocol recorded in manifests."""
...
def build_preparation_manifest(
    source_root: str | Path, config: dict[str, Any]
) -> dict[str, Any]:
    """Validate Stage-0 isolation and hash every source artifact."""
    validate_frozen_config(config)
    ...
    for seed in FROZEN_SEEDS:
        ...
        required_public = {"sample_id", "image_relpath", "noisy_label"}
        required_private = {"sample_id", "clean_label", "group", "is_noisy"}
        if not required_public.issubset(public.columns):
            raise ValueError(f"Public manifest schema mismatch: {public_path}")
        if not required_private.issubset(private.columns):
    ...
    return {
        ...
        "train_rows": int(len(canonical_train)),
...
def validate_result_frame(frame: pd.DataFrame, *, expected_seeds: Iterable[int]) -> None:
    """Validate the complete key space and metric invariants of result rows."""
    ...
    actual_keys = {
        (int(row.seed), str(row.method), str(row.noise_name), round(float(row.budget_fraction), 12))
    ...
    duplicate = frame.duplicated(["seed", "method", "noise_name", "budget_fraction"])
    if duplicate.any():
        raise ValueError("Duplicate confirmatory result keys")
    numeric = [
        "budget_count",
        "num_queried",
        "num_corrected",
    ...
    if not np.isfinite(frame[numeric].to_numpy(dtype=float)).all():
        raise ValueError("Confirmatory results contain non-finite primary values")
    if not np.allclose(
    ...
    for seed, group in frame.groupby("seed"):
        for column in ("baseline_wga", "baseline_average_accuracy", "baseline_balanced_accuracy"):
...
def _artifact_path(root: Path, value: Any) -> Path:
    ...
    try:
        ...
        raise ValueError(f"Artifact path escapes shard root: {raw}") from exc
    if not path.is_file():
...
def validate_seed_artifacts(
    ...
    """Recompute query composition and test metrics from persisted artifacts."""
    validate_result_frame(frame, expected_seeds=[seed])
    shard = Path(shard_root).resolve()
    source = Path(source_root).resolve()
    public = pd.read_csv(
        source / "manifests" / f"{FROZEN_NOISE_NAME}_seed{seed}_train_public.csv"
    ...
    baseline_prediction_paths = set()
    baseline_checkpoint_paths = set()
    ...
    for row in frame.to_dict("records"):
        ...
        if missing_ids:
            raise ValueError(
                "Query contains IDs outside the public train manifest: "
                f"{missing_ids[:5]}"
            )
        ...
        if not np.array_equal(prediction_ids, test_public["sample_id"].to_numpy(dtype=np.int64)):
            raise ValueError(f"Test prediction IDs are incomplete or reordered: {prediction_path}")
        ...
        if not np.array_equal(baseline_ids, test_public["sample_id"].to_numpy(dtype=np.int64)):
        ...
        for name, expected in baseline_metrics.items():
            _assert_close(
                f"seed {seed}/{method}/baseline_{name}",
...
def summarize_confirmatory_results(frame: pd.DataFrame) -> dict[str, Any]:
    """Apply the predeclared primary gate and report the paired Loss contrast."""
    validate_result_frame(frame, expected_seeds=FROZEN_SEEDS)
    candidate = frame[frame["method"] == PRIMARY_METHOD].sort_values("seed")
    loss = frame[frame["method"] == "loss"][ ["seed", "delta_wga"] ].rename(
        columns={"delta_wga": "loss_delta_wga"}
    )
    paired = candidate[["seed", "delta_wga"]].merge(
        loss, on="seed", how="inner", validate="one_to_one"
    )
    absolute = candidate["delta_wga"].to_numpy(dtype=float)
    relative = (paired["delta_wga"] - paired["loss_delta_wga"]).to_numpy(dtype=float)
    ...
    negative_count = int((absolute < 0).sum())
    mean_precision = float(candidate["noise_precision"].mean())
    gates = {
        "bootstrap_ci_upper_below_zero": bool(absolute_ci[1] < 0),
        "negative_seed_count_at_least_8": bool(
            negative_count >= PRIMARY_MIN_NEGATIVE_SEEDS
        ),
        "mean_noise_precision_at_least_0_99": bool(
            mean_precision >= PRIMARY_MIN_MEAN_NOISE_PRECISION
        ),
    }
    return {
        "protocol_version": PROTOCOL_VERSION,
        ...
        "seeds": list(FROZEN_SEEDS),
        "primary_estimand": "mean seed-paired delta_wga vs no correction",
        ...
        "mean_noise_precision": mean_precision,
        "exact_two_sided_sign_test_p": exact_two_sided_sign_test(absolute),
        "paired_vs_loss": {
            "mean_delta_wga_difference": float(relative.mean()),
            "bootstrap_95_ci": [float(relative_ci[0]), float(relative_ci[1])],
            "exact_two_sided_sign_test_p": exact_two_sided_sign_test(relative),
        },
        "primary_gate": gates,
