#!/usr/bin/env python
"""Run the sealed CelebA Young/Male RepairValue-Q replication protocol."""
...
# Required by deterministic CUDA matrix multiplications.  Set this before a
# CUDA context is created by preflight or formal training.
...
DEFAULT_CONFIG = REPOSITORY_ROOT / "configs" / "celeba_e2e_young_male_rvq_seed60_79.yaml"
...
def _environment() -> dict[str, Any]:
    ...
    code_paths = [
        REPOSITORY_ROOT / "src" / "robust_verify" / "celeba_young_male_confirmatory.py",
        ...
        REPOSITORY_ROOT / "src" / "robust_verify" / "oof.py",
...
def command_prepare(config: dict[str, Any]) -> dict[str, Any]:
    ...
    print(f"Generating fresh corruption manifests for seeds {FROZEN_SEEDS[0]}-{FROZEN_SEEDS[-1]}...")
...
def command_run(config: dict[str, Any], seeds: list[int] | None) -> list[Path]:
    ...
    if unknown:
        raise ValueError(f"Seeds are outside the frozen 60-79 set: {unknown}")
...
def _write_audit(output_root: Path, frame: pd.DataFrame) -> dict[str, Any]:
    ...
    audit = audit_absolute_utility(
        prepared,
        include_methods=list(FROZEN_METHODS),
    ...
    paired = decision["paired_vs_loss"]
    absolute = decision["absolute_utility"]
    lines = [
        "# CelebA Young/Male RepairValue-Q replication decision",
        "",
        ...
        "",
        (
            "- Mean paired Delta-WGA (RepairValue-Q minus Loss): "
        ...
        f"- Positive paired seeds: {paired['positive_seed_count']}/{len(FROZEN_SEEDS)}",
        "",
        (
            "- Absolute RepairValue-Q Delta-WGA: "
            f"{100 * absolute['mean_delta_wga']:+.3f} pp"
        ),
        (
            "- Absolute bootstrap 95% interval: "
        ...
        "",
        "The primary claim is the paired comparison against Loss; other methods are secondary baselines.",
        "",
    ]
...
def command_aggregate(config: dict[str, Any]) -> Path:
    ...
    for seed in FROZEN_SEEDS:
        ...
        for column in (*ARTIFACT_COLUMNS, OOF_ARTIFACT_COLUMN):
            frame[column] = frame[column].map(
                lambda value, prefix=shard_prefix: (
                    (prefix / str(value)).as_posix() if str(value) else ""
                )
            )
        ...
        for column in (*ARTIFACT_COLUMNS, OOF_ARTIFACT_COLUMN):
            values = [value for value in frame[column].tolist() if str(value)]
            files_to_hash.update(output_root / value for value in values)

    aggregate = pd.concat(frames, ignore_index=True).sort_values(["seed", "method"]).reset_index(drop=True)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--data-root", help="Server CelebA root; scientific paths are otherwise frozen")
