param(
    [string]$Destination = "dist/celeba_young_male_rvq_seed60_79_server.zip"
)
...
$items = @(
    ...
    "LICENSE",
    "README.md",
    "docs/CELEBA_YOUNG_MALE_REPLICATION.md",
    "configs/celeba_e2e_young_male_rvq_seed60_79.yaml",
    "configs/celeba_e2e_young_male_rvq_seed60_79.registration.json",
    "src/robust_verify",
    "scripts/run_celeba_young_male_server.py",
    "scripts/run_celeba_young_male_server.sh",
    "scripts/package_celeba_young_male_server.ps1",
    "tests/test_celeba_young_male_confirmatory.py",
    "tests/test_end_to_end_smoke.py",
    "tests/test_modern_baselines.py"
)

$staging = Join-Path ([System.IO.Path]::GetTempPath()) (
    "celeba_young_male_server_" + [guid]::NewGuid().ToString("N")
)
try {
    ...
    $manifestLines = Get-ChildItem -LiteralPath $staging -Recurse -File |
        ...
        ForEach-Object {
            # Windows PowerShell 5/.NET Framework has no Path.GetRelativePath.
