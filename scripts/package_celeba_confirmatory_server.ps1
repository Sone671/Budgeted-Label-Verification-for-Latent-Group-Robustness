param(
    [string]$Destination = "dist/celeba_confirmatory_seed10_19_server.zip"
)
...
$items = @(
    ...
    "LICENSE",
    "README.md",
    "CELEBA_CONFIRMATORY_SERVER_README.md",
    "configs/celeba_e2e_confirmatory_seed10_19.yaml",
    "src/robust_verify",
    "scripts/run_celeba_confirmatory_server.py",
    "scripts/run_celeba_confirmatory_server.sh",
    "tests/test_celeba_confirmatory.py",
    "tests/test_end_to_end_smoke.py",
    "tests/test_absolute_utility.py"
)

$staging = Join-Path ([System.IO.Path]::GetTempPath()) (
    "celeba_confirmatory_server_" + [guid]::NewGuid().ToString("N")
)
try {
