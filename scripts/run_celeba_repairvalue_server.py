#!/usr/bin/env python
"""Prepare, run, resume, aggregate, and audit the frozen RepairValue follow-up."""
...
from robust_verify.celeba_repairvalue_confirmatory import (
    ARTIFACT_COLUMNS,
    FROZEN_SEEDS,
    PRIMARY_METHOD,
    PROTOCOL_VERSION,
...
DEFAULT_CONFIG = REPOSITORY_ROOT / "configs" / "celeba_e2e_repairvalue_seed20_29.yaml"
...
def _environment() -> dict[str, Any]:
    ...
    code_paths = [
        REPOSITORY_ROOT / "src" / "robust_verify" / "celeba_repairvalue_confirmatory.py",
...
def command_run(config: dict[str, Any], seeds: list[int] | None) -> list[Path]:
    ...
    if unknown:
        raise ValueError(f"Seeds are outside the frozen 20-29 set: {unknown}")
...
def _write_audit(output_root: Path, frame: pd.DataFrame) -> dict[str, Any]:
    ...
    audit = audit_absolute_utility(
        prepared,
        include_methods=[PRIMARY_METHOD, "tracin_val_uncertainty"],
        n_bootstrap=10_000,
        rng_seed=20260802,
    )
    ...
    absolute = decision["absolute_secondary"]
    lines = [
        "# CelebA RepairValue-Q prospective follow-up decision",
        "",
        ...
        f"- Positive paired seeds: {paired['positive_seed_count']}/10",
        "",
        (
        ...
        "",
        (
            "The primary claim is the paired comparison against Loss. Absolute "
            "utility, TracIn, precision, and group composition are secondary."
        ),
        "",
    ]
