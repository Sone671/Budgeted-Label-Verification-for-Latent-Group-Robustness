#!/usr/bin/env python
"""Run and audit the frozen Waterbirds RepairValue-Q seed-20--39 protocol."""
...
DEFAULT_CONFIG = REPOSITORY_ROOT / "configs" / "waterbirds_e2e_repairvalue_seed20_39.yaml"
...
def _environment() -> dict[str, Any]:
    ...
    code_paths = [
        REPOSITORY_ROOT / "src" / "robust_verify" / "waterbirds_repairvalue_confirmatory.py",
...
def command_run(config: dict[str, Any], seeds: list[int] | None) -> list[Path]:
    ...
    if unknown:
        raise ValueError(f"Seeds are outside the frozen 20-39 set: {unknown}")
...
def _write_audit(output_root: Path, frame: pd.DataFrame) -> dict[str, Any]:
    ...
    lines = [
        "# Waterbirds RepairValue-Q prospective confirmation decision",
        "",
        ...
        (
            ...
            "the paired CI, at least 15/20 positive paired seeds, and a positive "
            "absolute CI. Random, NoiseScore, TracIn, precision, and group "
