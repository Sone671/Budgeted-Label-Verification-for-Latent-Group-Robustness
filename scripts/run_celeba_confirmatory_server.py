#!/usr/bin/env python
"""Prepare, run, resume, aggregate, and audit the frozen CelebA server job."""
...
# Required by deterministic CUDA matrix multiplications.  Set it before the
# first CUDA context is created by the preflight batch or formal training.
...
DEFAULT_CONFIG = REPOSITORY_ROOT / "configs" / "celeba_e2e_confirmatory_seed10_19.yaml"
MIN_CUDA_MEMORY_GIB = 18.0
...
def _data_preflight(config: dict[str, Any]) -> dict[str, Any]:
    ...
    partition = root / str(data["partition_csv"])
    image_root = root / str(data["image_subdir"])
    for path in (metadata, partition):
        if not path.is_file():
            raise FileNotFoundError(f"Required CelebA file is missing: {path}")
    if not image_root.is_dir():
        raise FileNotFoundError(f"CelebA image directory is missing: {image_root}")

    metadata_head = pd.read_csv(metadata, nrows=1)
    partition_head = pd.read_csv(partition, nrows=1)
    required_metadata = {"image_id", data["target_attr"], data["spurious_attr"]}
    if not required_metadata.issubset(metadata_head.columns):
        raise ValueError(f"CelebA metadata lacks columns: {sorted(required_metadata)}")
    if not {"image_id", "partition"}.issubset(partition_head.columns):
        raise ValueError("CelebA partition CSV must contain image_id and partition")
    first_image = image_root / str(metadata_head.iloc[0]["image_id"])
    if not first_image.is_file():
        raise FileNotFoundError(f"First CelebA image is unavailable: {first_image}")
...
def _gpu_preflight(config: dict[str, Any], *, run_batch_smoke: bool) -> dict[str, Any]:
    if not torch.cuda.is_available():
        raise RuntimeError("The formal run requires a CUDA GPU; torch.cuda.is_available() is false")
    device_index = torch.cuda.current_device()
    properties = torch.cuda.get_device_properties(device_index)
    total_gib = properties.total_memory / (1024**3)
    if total_gib < MIN_CUDA_MEMORY_GIB:
        raise RuntimeError(
            f"The formal configuration requires at least {MIN_CUDA_MEMORY_GIB:.0f} GiB VRAM; "
    ...
    if run_batch_smoke:
        ...
        model.train().cuda()
        images = torch.randn(
            int(e2e["batch_size"]), 3, int(e2e["input_size"]), int(e2e["input_size"]),
            device="cuda",
        )
...
def _environment() -> dict[str, Any]:
    code_paths = [
        REPOSITORY_ROOT / "src" / "robust_verify" / "celeba_confirmatory.py",
        REPOSITORY_ROOT / "src" / "robust_verify" / "end_to_end.py",
        REPOSITORY_ROOT / "src" / "robust_verify" / "scoring.py",
        REPOSITORY_ROOT / "src" / "robust_verify" / "influence.py",
...
def command_preflight(config: dict[str, Any], *, gpu_smoke: bool = True) -> dict[str, Any]:
    report = {
        "protocol_version": PROTOCOL_VERSION,
        "protocol_sha256": protocol_sha256(config),
        "data": _data_preflight(config),
        "gpu": _gpu_preflight(config, run_batch_smoke=gpu_smoke),
        "environment": _environment(),
    }
    output = _output_root(config) / "preflight.json"
    _write_json(output, report)
    print(
        f"Preflight PASS: {report['gpu']['gpu_name']} "
        f"({report['gpu']['cuda_memory_gib']:.1f} GiB)"
    )
...
def command_prepare(config: dict[str, Any]) -> dict[str, Any]:
    ...
    print(
        "Generating held-out corruption manifests for seeds "
        f"{FROZEN_SEEDS[0]}-{FROZEN_SEEDS[-1]}..."
    )
...
def _load_preparation(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not marker.is_file():
        raise RuntimeError(f"Prepared source is absent; run the prepare command first: {marker}")
    manifest = json.loads(marker.read_text(encoding="utf-8"))
    verify_preparation_manifest(source, manifest, config)
    return manifest
...
def _next_attempt(output_root: Path, seed: int) -> Path:
    existing = _attempts(output_root, seed)
    indices = []
    ...
    try:
        ...
        complete = {
            **run_manifest,
            ...
            "results": str(results_path.relative_to(attempt).as_posix()),
...
def command_run(config: dict[str, Any], seeds: list[int] | None) -> list[Path]:
    ...
    if unknown:
        raise ValueError(f"Seeds are outside the frozen 10-19 set: {unknown}")
...
def _write_audit(output_root: Path, frame: pd.DataFrame) -> dict[str, Any]:
    ...
    prepared = prepare_results(frame, "celeba")
    audit = audit_absolute_utility(
        prepared,
        include_methods=[PRIMARY_METHOD],
        n_bootstrap=10_000,
        rng_seed=20260724,
    )
    ...
    lines = [
        "# CelebA held-out confirmatory decision",
        "",
        f"Protocol: `{PROTOCOL_VERSION}`",
        "",
        f"Primary gate: **{'PASS' if decision['primary_gate_passed'] else 'NOT PASSED'}**",
        "",
        f"- Mean absolute Delta-WGA: {100 * decision['mean_delta_wga']:+.3f} pp",
        (
            "- Seed-bootstrap 95% interval: "
            f"[{100 * decision['bootstrap_95_ci'][0]:+.3f}, "
            f"{100 * decision['bootstrap_95_ci'][1]:+.3f}] pp"
        ),
        f"- Negative seeds: {decision['negative_seed_count']}/10",
        f"- Mean query noise precision: {100 * decision['mean_noise_precision']:.3f}%",
        "",
        (
            "The relative-to-Loss contrast is secondary and is not substituted "
...
def command_aggregate(config: dict[str, Any]) -> Path:
    ...
    for seed in FROZEN_SEEDS:
        ...
        if len(completed) != 1:
            raise RuntimeError(
                f"Seed {seed} must have exactly one completed attempt before aggregation; "
                f"found {completed}"
            )
        attempt = completed[0]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "command", choices=("preflight", "prepare", "run", "aggregate", "all")
    )
    parser.add_argument("--config", default=str(DEFAULT_CONFIG))
    parser.add_argument("--data-root", help="Server CelebA root; paths are the only override")
...
def main() -> None:
    ...
    if args.command == "preflight":
        ...
        if args.seed:
            print(
                "Selected-seed run complete; aggregation is deferred until "
                "all seeds 10-19 finish"
            )
        else:
            command_aggregate(config)
