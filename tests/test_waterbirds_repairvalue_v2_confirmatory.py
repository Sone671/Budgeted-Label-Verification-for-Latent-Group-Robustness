...
from robust_verify.waterbirds_repairvalue_v2_confirmatory import (
    FROZEN_METHODS,
    FROZEN_SEEDS,
...
CONFIG_PATH = "configs/waterbirds_e2e_repairvalue_v2_seed40_59.yaml"
...
def _result_frame(repair_delta: float = 0.02) -> pd.DataFrame:
    deltas = {
        ...
        "loss": 0.005,
        "noise_score": 0.010,
        "expected_repair_value": 0.005,
        "noise_gated_repair_value": repair_delta,
    }
    precisions = {
        "random": 0.20,
        "loss": 0.90,
        "noise_score": 0.70,
        "expected_repair_value": 0.10,
        "noise_gated_repair_value": 0.65,
    }
    rows = []
            ...
            rows.append(
                {
                    ...
                    "num_queried": 100,
                    "num_corrected": 20,
                    "noise_precision": precisions[method],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("methods", ["noise_score", "noise_gated_repair_value"]),
        ("budgets", [0.02]),
...
def test_confirmatory_summary_applies_registered_paired_gate() -> None:
    ...
    assert summary["paired_vs_noise_score"]["mean_delta_wga_difference"] == pytest.approx(
        0.010
    )
    assert summary["paired_vs_noise_score"]["bootstrap_95_ci"] == pytest.approx([0.010, 0.010])
    assert summary["paired_vs_noise_score"]["positive_seed_count"] == 20
    assert summary["absolute_utility"]["mean_delta_wga"] == pytest.approx(0.02)
    assert summary["precision_noninferiority"]["precision_drop"] == pytest.approx(0.05)
    assert summary["precision_noninferiority"][
...
def test_confirmatory_summary_retains_a_failed_null_result() -> None:
    ...
    for seed in (40, 41, 42, 43, 44, 45):
        frame.loc[
            frame["seed"].eq(seed) & frame["method"].eq("noise_gated_repair_value"),
    ...
    assert summary["paired_vs_noise_score"]["positive_seed_count"] == 14
...
def test_confirmatory_summary_requires_positive_absolute_utility() -> None:
    frame = _result_frame()
    for method, delta in (("noise_score", -0.02), ("noise_gated_repair_value", -0.005)):
...
def test_confirmatory_summary_requires_precision_noninferiority() -> None:
    frame = _result_frame()
    frame.loc[frame["method"].eq("noise_gated_repair_value"), "noise_precision"] = 0.50

    summary = summarize_confirmatory_results(frame)

    assert summary["precision_noninferiority"]["precision_drop"] == pytest.approx(0.20)
    assert summary["primary_gate"][
        "precision_noninferiority_ci_lower_at_least_negative_10pp"
    ] is False
    assert summary["primary_gate_passed"] is False
