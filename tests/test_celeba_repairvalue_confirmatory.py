...
CONFIG_PATH = "configs/celeba_e2e_repairvalue_seed20_29.yaml"
...
def test_frozen_config_rejects_scientific_changes(field: str, value: object) -> None:
    ...
    with pytest.raises(ValueError, match="Frozen CelebA RepairValue protocol mismatch"):
        validate_frozen_config(config)
...
def test_confirmatory_summary_applies_registered_paired_gate() -> None:
    ...
    assert summary["paired_vs_loss"]["positive_seed_count"] == 10
...
def test_confirmatory_summary_retains_a_failed_null_result() -> None:
    ...
    assert summary["paired_vs_loss"]["positive_seed_count"] == 7
    assert summary["primary_gate"]["positive_paired_seed_count_at_least_8"] is False
...
def test_group_mechanism_audit_uses_each_seed_baseline_worst_group(tmp_path) -> None:
    ...
    assert audit["weak_group_accuracy_change"].to_numpy() == pytest.approx(np.full(40, 0.05))
